...
import { Building2, CalendarDays, ExternalLink, Newspaper } from 'lucide-vue-next';
        ...
        <div class="border-b border-gray-100 p-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex min-w-0 items-center gap-3">
                    <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-800">
                        <Newspaper class="h-6 w-6" />
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-bold text-orange-500">آخر الأخبار</p>
                        <h2 class="truncate text-lg font-extrabold text-blue-800">
            ...
            >
                <span class="shrink-0 rounded-full bg-orange-500 px-2 py-0.5 text-[11px] text-white">
                    الأحدث
                </span>
        ...
        <div v-if="items?.length" class="overflow-x-auto overscroll-x-contain">
            <div class="flex w-max gap-4 p-4">
                ...
                >
                    ...
                    >
                        <div class="h-24 overflow-hidden rounded-xl bg-gray-100">
                            <img
                                v-if="item.image_url"
                                :src="item.image_url"
                                :alt="item.title"
                            ...
                            />
                            <div v-else class="flex h-full items-center justify-center text-blue-800">
                                <Newspaper class="h-7 w-7" />
                            </div>
                        </div>

                        <div class="min-w-0">
                            <h3 class="line-clamp-2 text-sm font-extrabold leading-6 text-gray-950 group-hover:text-orange-600">
                                {{ item.title }}
                            </h3>

                            <div class="mt-2 space-y-1 text-[11px] font-bold text-gray-500">
                                <span v-if="item.author" class="flex min-w-0 items-center gap-1">
                                    <Building2 class="h-3.5 w-3.5 shrink-0 text-blue-800" />
                                    <span class="truncate">{{ item.author }}</span>
                                </span>
                                <span v-if="item.published_at" class="flex items-center gap-1">
                                    <CalendarDays class="h-3.5 w-3.5 text-orange-500" />
                                    {{ item.published_at }}
                                </span>
                            </div>
                        </div>
