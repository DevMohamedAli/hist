...
<template>
    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm" dir="rtl">
        <div class="border-b border-gray-100 bg-slate-50/80 px-5 py-4 sm:px-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="min-w-0">
                    <p class="text-xs font-bold text-orange-600">معالج الاستيراد</p>
                    <h2 class="mt-1 text-xl font-extrabold text-blue-900 sm:text-2xl">
        ...
        <div class="space-y-5 p-5 sm:p-6">
            ...
            <div v-if="currentStep === 1" class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
                <button
                    type="button"
                ...
                >
                    ...
                    />
                    <span class="rounded-full bg-blue-50 p-4 text-blue-700 ring-1 ring-blue-100 transition group-hover:bg-white">
                        <Loader2 v-if="isUploading" class="h-10 w-10 animate-spin" />
                        <UploadCloud v-else class="h-10 w-10" />
                    </span>
                    <span class="mt-5 text-lg font-extrabold text-gray-900">
                        {{ isUploading ? 'جاري رفع الملف...' : 'اسحب الملف هنا أو انقر للاختيار' }}
                    </span>
                    <span class="mt-2 text-sm text-gray-500">CSV أو XLS أو XLSX</span>
                </button>

                <aside class="rounded-lg border border-gray-200 bg-slate-50 p-4">
                    <label class="block text-sm font-bold text-gray-800">نوع البيانات</label>
                    <select
                    ...
                    >
                        <option v-for="type in importTypes" :key="type.value" :value="type.value">
                            {{ type.label }}
                        </option>
                    </select>
            ...
            <div v-if="currentStep === 2 && !isWorkbookImport" class="space-y-5">
                <div class="grid gap-4 sm:grid-cols-3">
                    <div class="rounded-md border border-blue-100 bg-blue-50 p-4">
                        ...
                        <p class="mt-1 text-2xl font-extrabold text-blue-900">{{ columns.length }}</p>
                    </div>
                    <div class="rounded-md border border-green-100 bg-green-50 p-4">
                        <p class="text-xs font-bold text-green-700">حقول مربوطة</p>
                        <p class="mt-1 text-2xl font-extrabold text-green-900">{{ mappedCount }}</p>
                    </div>
                    <div class="rounded-md border border-gray-200 bg-gray-50 p-4">
                        <p class="text-xs font-bold text-gray-600">صفوف المعاينة</p>
                        <p class="mt-1 text-2xl font-extrabold text-gray-900">{{ previewData.length }}</p>
                    </div>
                </div>

                <div v-if="isLoadingPreview" class="flex items-center justify-center gap-2 rounded-lg border p-10 text-blue-700">
            ...
            <div v-if="currentStep === 3 && !isWorkbookImport" class="space-y-4">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">اختر الصفوف التي تريد تنفيذ الاستيراد عليها.</p>
                    </div>
                    <label class="inline-flex items-center gap-2 text-sm font-bold text-blue-800">
                        <input v-model="selectAll" type="checkbox" class="rounded border-gray-300" @change="toggleAllRows" />
                        تحديد الكل
                    </label>
                </div>

                <div class="max-h-[460px] overflow-auto rounded-lg border border-gray-200">
                    <table class="min-w-full text-right text-sm">
                        <thead class="sticky top-0 bg-gray-100 text-gray-700">
                            <tr>
                                <th class="w-12 border-b px-3 py-3"></th>
                                <th v-for="col in columns" :key="col" class="border-b px-3 py-3 whitespace-nowrap">
                                    {{ col }}
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                            <tr v-for="(row, rowIndex) in previewRows" :key="rowIndex" class="hover:bg-blue-50/40">
                                <td class="px-3 py-3">
                                    <input v-model="selectedRows[rowIndex]" type="checkbox" class="rounded border-gray-300" />
                                </td>
                                <td v-for="col in columns" :key="col" class="max-w-56 truncate px-3 py-3 text-gray-700">
                                    {{ row[col] ?? '-' }}
                                </td>
                            </tr>
                        </tbody>
            ...
            <div v-if="currentStep === 4" class="space-y-5">
                ...
                <div v-else class="grid gap-4 sm:grid-cols-3">
                    ...
                    <div class="rounded-md border border-red-200 bg-red-50 p-5 text-center">
                        ...
                        <p class="mt-1 text-3xl font-extrabold text-red-900">{{ errorCount }}</p>
                    </div>
                    <div class="rounded-md border border-blue-200 bg-blue-50 p-5 text-center">
                        <p class="text-sm font-bold text-blue-700">محدد للاستيراد</p>
                        <p class="mt-1 text-3xl font-extrabold text-blue-900">{{ selectedCount }}</p>
                    </div>
                </div>
        ...
        <div class="sticky bottom-0 flex items-center justify-between border-t border-gray-200 bg-white/95 px-5 py-4 backdrop-blur sm:px-6">
            <button
                type="button"
