...
const userPermissions = computed(() => {
    ...
    return Array.isArray(permissions)
        ? permissions.map((permission) => (typeof permission === 'string' ? permission : permission.name))
        : []
})

const hasRole = (...roles: string[]) => {
    return userRoles.value.includes('super_admin') || roles.some((role) => userRoles.value.includes(role))
}

const hasPermission = (permission: string) => {
    return userRoles.value.includes('super_admin') || userPermissions.value.includes(permission)
}
...
const userAvatar = computed(() => (typeof user.value?.avatar === 'string' && user.value.avatar.trim() !== '' ? user.value.avatar : null))
        ...
        items: [
            {
                ...
                activePrefixes: ['/dashboard', '/employee/dashboard', '/teacher/dashboard', '/student/dashboard'],
            },
        ],
    },
    {
    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                icon: Settings,
                activePrefixes: ['/settings/profile', '/settings/security', '/settings/appearance'],
            },
        ],
    },
]
...
const isActive = (item: NavItem) =>
    item.activePrefixes.some((prefix) => page.url === prefix || page.url.startsWith(`${prefix}/`))
...
const currentSection = computed(() =>
    visibleNavigation.value.find((section) => section.items.some((item) => isActive(item))),
)
...
<template>
    <div class="min-h-screen bg-slate-50 font-['Cairo'] text-slate-950" dir="rtl">
        <div v-if="sidebarOpen" class="fixed inset-0 z-40 bg-slate-950/45 backdrop-blur-sm lg:hidden"
            @click="closeSidebar" />
        ...
        <aside
            ...
            <div class="border-b border-white/10 p-4">
                <div class="flex items-start justify-between gap-3">
                    <Link href="/dashboard" class="flex min-w-0 items-start gap-3" @click="closeSidebar">
                        <AppLogoIcon class="h-11 w-11 shrink-0 rounded-xl" />
                        <div v-show="!desktopCollapsed" class="min-w-0">
                            <p class="text-sm leading-6 font-extrabold">المعهد العالي للعلوم والتقنية</p>
                            <p class="text-sm font-bold text-orange-200">العجيلات</p>
                        </div>
                    </Link>
                    ...
                    <div class="flex items-center gap-1">
                        <button type="button"
                            ...
                            :title="desktopCollapsed ? 'توسيع القائمة' : 'طي القائمة'"
                            @click="desktopCollapsed = !desktopCollapsed">
                            <PanelRightClose class="h-5 w-5" />
                        </button>
                        <button type="button" class="rounded-lg p-2 text-blue-100 hover:bg-white/10 lg:hidden"
                            @click="closeSidebar">
                            <X class="h-5 w-5" />
                        </button>
                    </div>
            ...
            <nav class="flex-1 space-y-5 overflow-y-auto px-3 py-5">
                <section v-for="section in visibleNavigation" :key="section.group">
                    <h2 v-show="!desktopCollapsed" class="mb-2 px-3 text-xs font-extrabold tracking-wide text-blue-100">
                        {{ section.group }}
                    </h2>
                    ...
                    <div class="space-y-1">
                        <Link v-for="item in section.items" :key="item.title" :href="item.href"
                            ...
                            :class="isActive(item)
                                ...
                                : 'text-blue-50 hover:bg-white/10'" :title="desktopCollapsed ? item.title : undefined"
                            @click="closeSidebar">
                            <component :is="item.icon" class="h-5 w-5 shrink-0" />
                            <span v-show="!desktopCollapsed" class="truncate">{{ item.title }}</span>
                        </Link>
                    </div>
                </section>
            </nav>
            ...
            <div class="border-t border-white/10 p-3">
                <div class="flex items-center gap-3 rounded-xl bg-white/10 p-3"
                    ...
                    <div v-show="!desktopCollapsed" class="min-w-0">
                        <p class="truncate text-sm font-extrabold">{{ user?.name || 'المستخدم' }}</p>
                        <p class="truncate text-xs text-blue-100">{{ displayRoles.join('، ') || 'حساب النظام' }}</p>
                    </div>
                </div>
            </div>
        </aside>

        <div class="transition-all duration-200" :class="desktopCollapsed ? 'lg:ps-20' : 'lg:ps-80'">
                ...
                <div class="flex h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0 text-start">
                            ...
                            <h1 class="truncate text-lg font-extrabold text-blue-900">
                                {{ currentItem?.title ?? 'منظومة الإدارة الأكاديمية' }}
                            </h1>
                        </div>
                    </div>
                    ...
                    <div v-if="clockReady"
                        ...
                        <div class="leading-tight">
                            <p class="text-xs font-extrabold text-blue-950">{{ arabicDate }}</p>
                            <p class="mt-0.5 text-xs font-bold text-orange-600">{{ arabicTime }}</p>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div v-if="profileOpen"
                            ...
                            <div class="border-b border-gray-100 px-4 py-3 text-start">
