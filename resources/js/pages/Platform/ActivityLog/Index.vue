...
const todayCount = computed(() => {
    ...
    return props.activities.data.filter((a) => a.created_at.startsWith(today))
        .length;
});
...
<template>
    ...
    <div
        class="activity-log-page min-h-screen bg-linear-to-br from-slate-50 to-blue-50/30 p-4 md:p-6 dark:from-gray-900 dark:to-gray-800"
        dir="rtl"
    >
        ...
        >
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-blue-100">
                        مراقبة وتحليل جميع نشاطات النظام بلحظة
                    </p>
                </div>
            ...
            <div class="mt-6 flex flex-wrap gap-4">
                <div
                    class="flex items-center gap-2 rounded-full bg-white/20 px-4 py-1.5 backdrop-blur-sm"
                >
                    <Activity class="h-4 w-4 text-white" />
                    <span class="text-sm font-semibold text-white"
                        >{{ totalActivities.toLocaleString() }} إجمالي</span
                    >
                </div>
                <div
                    class="flex items-center gap-2 rounded-full bg-white/20 px-4 py-1.5 backdrop-blur-sm"
                ...
                >
                    ...
                    <span class="text-sm font-semibold text-white"
                        >{{ props.causers.length }} مستخدمين</span
                    >
                </div>
            </div>
        </div>
        ...
        >
            ...
            >
                <CardTitle class="flex items-center gap-2 text-xl">
                    ...
                    قائمة النشاطات
                    <span
                        class="mr-2 rounded-full bg-orange-100 px-2 py-0.5 text-xs font-normal text-orange-700"
                    >
        ...
        <Transition name="float-button">
            <button
                v-if="isScrolled"
                @click="exportCSV"
                class="fixed bottom-6 left-6 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-orange-500 text-white shadow-2xl transition-all hover:scale-110 hover:bg-orange-600"
                title="تصدير CSV"
            >
