...
import { Eye, Calendar, User, Sparkles, ChevronLeft, ChevronRight } from 'lucide-vue-next';
...
const getEventBadgeClass = (event: string) => {
    const map: Record<string, string> = {
        created: 'bg-gradient-to-r from-emerald-400 to-emerald-500 text-white border-0 shadow-sm',
        updated: 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white border-0 shadow-sm',
        deleted: 'bg-gradient-to-r from-rose-500 to-red-600 text-white border-0 shadow-sm',
        restored: 'bg-gradient-to-r from-amber-400 to-yellow-500 text-white border-0 shadow-sm',
    ...
    return map[event] || 'bg-gradient-to-r from-orange-400 to-orange-600 text-white border-0 shadow-sm';
};
...
<template>
    <div class="activity-table-component w-full">
            ...
            <Table>
                <TableHeader>
                    <TableRow
                        ...
                        <TableHead class="w-32 font-semibold">المستخدم</TableHead>
                        <TableHead class="w-44 font-semibold">التاريخ</TableHead>
                ...
                <TableBody>
                    ...
                    <template v-if="loading">
                        <TableRow v-for="i in skeletonRows" :key="i" class="animate-pulse">
                            <TableCell>
                    ...
                    <template v-else>
                        <TableRow v-for="(activity, idx) in activities.data" :key="activity.id"
                            class="group transition-all duration-300 hover:bg-linear-to-r hover:from-orange-50/50 hover:to-transparent dark:hover:from-orange-900/20 cursor-pointer"
                            :style="{ animationDelay: `${idx * 30}ms` }">
                            <TableCell>
                                <Badge :class="getEventBadgeClass(activity.event)"
                                    class="font-semibold shadow-sm transition-transform group-hover:scale-105">
                                    {{ activity.event || 'غير معروف' }}
                                </Badge>
                            </TableCell>
                            <TableCell>
                                <span class="line-clamp-2 text-gray-700 dark:text-gray-300">{{ activity.description
                                }}</span>
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell class="whitespace-nowrap">
                                <div class="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400">
                                    ...
                                    <span class="hidden sm:inline text-xs text-gray-400">({{
                                        formatDate(activity.created_at) }})</span>
                                </div>
                            </TableCell>
                            <TableCell>
                                <Button variant="ghost" size="icon" @click="emit('view-detail', activity)"
                                    class="transition-all duration-200 hover:scale-110 hover:bg-orange-100 dark:hover:bg-orange-900/30">
                                    <Eye class="h-4 w-4 text-gray-600 hover:text-orange-600 dark:text-gray-400" />
                                </Button>
                            </TableCell>
                        </TableRow>
                    </template>
                    ...
                    <TableRow v-if="!loading && activities.data.length === 0">
                        <TableCell colspan="5" class="py-16 text-center">
                            <div class="flex flex-col items-center justify-center gap-3">
                                <Sparkles class="h-12 w-12 text-gray-300 dark:text-gray-600" />
                                <p class="text-lg font-medium text-gray-500 dark:text-gray-400">لا توجد نشاطات</p>
                                <p class="text-sm text-gray-400 dark:text-gray-500">حاول تعديل الفلاتر أو إجراء نشاط
                                    جديد</p>
                            </div>
                        </TableCell>
                    </TableRow>
        ...
        <div class="md:hidden space-y-3">
            ...
            <template v-if="loading">
                <div v-for="i in skeletonRows" :key="i"
                    ...
                    <div class="mt-3 h-4 w-full rounded bg-gray-200 dark:bg-gray-700"></div>
            ...
            <!-- Real data cards -->
            <div v-for="(activity, idx) in activities.data" :key="activity.id"
                ...
                <div class="flex items-start justify-between">
                    <Badge :class="getEventBadgeClass(activity.event)" class="shadow-sm">
                        {{ activity.event || 'غير معروف' }}
                    </Badge>
                    <Button variant="ghost" size="icon" @click="emit('view-detail', activity)"
                ...
                <p class="mt-3 line-clamp-3 text-sm text-gray-700 dark:text-gray-300">{{ activity.description }}</p>
            ...
            <!-- Empty state mobile -->
            <div v-if="!loading && activities.data.length === 0" class="py-16 text-center">
        ...
        <!-- Animated Pagination -->
        <div v-if="activities.last_page > 1 && !loading"
            ...
            <div class="flex items-center gap-2">
                <Button variant="outline" size="sm" :disabled="activities.current_page === 1"
                    @click="changePage(activities.links.find(l => l.label === '&laquo; Previous')?.url || null)"
                ...
                <div class="flex items-center gap-1">
                    ...
                    <span class="text-sm text-gray-500">من {{ activities.last_page }}</span>
                </div>
                <Button variant="outline" size="sm" :disabled="activities.current_page === activities.last_page"
                    @click="changePage(activities.links.find(l => l.label === 'Next &raquo;')?.url || null)"
                    class="transition-all hover:scale-105 disabled:opacity-50">
                    التالي
