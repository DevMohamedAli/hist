...
const canSubmit = computed(
    () => selectedFile.value && !isPreviewing.value && !isImporting.value,
);
...
const sendPdf = async (preview: boolean) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                payload.error || payload.message || 'تعذر تنفيذ العملية.',
            );
        }
        ...
        if (preview) {
            ...
            successMessage.value =
                payload.message || 'تم استيراد المقررات بنجاح.';
    ...
    <main class="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8" dir="rtl">
        <div class="mx-auto max-w-6xl space-y-6">
                ...
                >
                    <div>
                        ...
                        <h1 class="mt-1 text-2xl font-bold">
                            استيراد مقررات HISD من PDF
                        </h1>
                        <p class="mt-2 text-sm text-blue-100">
                            ارفع ملف PDF يحتوي على جدول المقررات وسيتم استخراج
                            الرمز والاسم والوحدات وحفظها في جدول المقررات.
                        </p>
                    </div>
            ...
            <section class="rounded-xl border bg-white p-6 shadow-sm">
                <div class="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
                    <div class="space-y-2">
                        ...
                        <p class="text-xs text-slate-500">
                            يدعم النصوص العربية والإنجليزية قدر الإمكان. ملفات
                            PDF المصورة تحتاج OCR قبل الاستيراد.
                        </p>
                    </div>
                ...
                <div
                    v-if="isPreviewing || isImporting || progress > 0"
                    class="mt-5"
                >
                    <div class="h-2 overflow-hidden rounded-full bg-slate-100">
                        <div
                            class="h-full bg-blue-700 transition-all"
                ...
                <div
                    v-if="errorMessage"
                    class="mt-5 rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700"
                >
                    <div class="flex items-center gap-2">
            ...
            >
                <div class="border-b p-4">
                    ...
                    <p class="text-sm text-slate-500">
                        تأكد من البيانات قبل الاستيراد، خصوصاً ملفات PDF ذات
                        الأعمدة المتعددة.
                    </p>
                </div>
            ...
            >
                ...
                <ul
                    class="mt-3 list-inside list-disc space-y-1 text-sm text-orange-700"
                >
                    <li v-for="(error, index) in summary.errors" :key="index">
                        {{ error }}
                    </li>
                </ul>
            </section>
