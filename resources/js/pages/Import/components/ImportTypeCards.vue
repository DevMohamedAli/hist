...
import { BookOpen, CalendarDays, GraduationCap, Users } from 'lucide-vue-next'
    ...
    <section class="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
        ...
        >
            <div
                ...
                :class="model === type.value ? 'bg-blue-700 text-white' : 'bg-blue-50 text-blue-800'"
            >
                <component :is="icons[type.value] ?? Users" class="h-5 w-5" />
            </div>
            <div class="min-w-0">
                <p class="truncate text-sm font-extrabold text-gray-900">{{ type.label }}</p>
