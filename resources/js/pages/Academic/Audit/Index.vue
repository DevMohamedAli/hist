...
const checkLabels: Record<string, string> = {
    ...
    course_classes_without_instructor_or_study_group: 'شعب بدون محاضر أو مجموعة',
    students_with_invalid_status_for_enrollments: 'طلاب بحالة غير مناسبة ولديهم تنزيلات',
        ...
        <div class="mx-auto max-w-7xl space-y-6">
            <section class="border-b border-slate-200 pb-5">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <p class="text-sm font-bold text-orange-600">مراقبة النظام الأكاديمي</p>
                        <h1 class="mt-1 text-2xl font-black text-blue-950">سلامة البيانات الأكاديمية</h1>
                        <p class="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
                            مراجعة مباشرة للبيانات الأساسية التي تؤثر على التسجيل، رصد الدرجات، الترفيع، والتخرج.
                        </p>
                    </div>
            ...
            <section
                ...
                :class="report.has_issues ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'"
            >
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div class="flex items-start gap-3">
                        <component
                            :is="report.has_issues ? AlertTriangle : ShieldCheck"
                            class="mt-1 h-7 w-7 shrink-0"
                            :class="report.has_issues ? 'text-amber-600' : 'text-emerald-600'"
                        />
                        <div>
                            <h2 class="text-lg font-black" :class="report.has_issues ? 'text-amber-950' : 'text-emerald-950'">
                                {{ report.has_issues ? 'توجد ملاحظات تحتاج مراجعة' : 'لا توجد مشاكل مانعة' }}
                            </h2>
                            <p class="mt-1 text-sm" :class="report.has_issues ? 'text-amber-800' : 'text-emerald-800'">
                                آخر تحديث: {{ generatedAt }}
                            </p>
                        </div>
                    </div>

                    <div class="rounded-md bg-white/70 px-4 py-3 text-center shadow-sm">
                        <p class="text-xs font-bold text-slate-500">إجمالي الملاحظات</p>
                        <p class="mt-1 text-2xl font-black text-slate-950">{{ totalIssues }}</p>
                    </div>
                </div>
            </section>
                ...
                >
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-2 text-2xl font-black text-blue-950">{{ metric.value }}</p>
                        </div>
            ...
            <section class="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
                <div class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-black text-slate-950">جاهزية بيانات التجربة</h2>
                    </div>
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        >
                            ...
                            <span
                                ...
                                :class="item.ready ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'"
                            >
                                <CheckCircle2 v-if="item.ready" class="h-3.5 w-3.5" />
                ...
                <div class="rounded-md border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-100 p-5">
                        <h2 class="text-base font-black text-slate-950">فحوصات سلامة البيانات</h2>
                        <p class="mt-1 text-sm text-slate-500">اضغط على أي فحص يحتوي ملاحظات لعرض التفاصيل.</p>
                    </div>

                    <div class="divide-y divide-slate-100">
                        <article v-for="check in checks" :key="check.key" class="p-4">
                            <button
                                type="button"
                            ...
                            >
                                ...
                                <span
                                    ...
                                    :class="check.count > 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'"
                                >
                                    {{ check.count }}
                                </span>
                            </button>

                            <div v-if="openChecks.includes(check.key)" class="mt-3 overflow-hidden rounded-md border border-slate-100">
                                <div v-if="check.rows.length === 0" class="bg-slate-50 px-4 py-3 text-sm font-bold text-slate-500">
                                    لا توجد سجلات مخالفة في هذا الفحص.
                                </div>

                                <div v-else class="max-h-80 overflow-auto bg-slate-950 text-left text-xs text-slate-100" dir="ltr">
                                    <div v-for="(row, index) in check.rows" :key="index" class="border-b border-white/10 p-3">
                                        <dl class="grid gap-2 sm:grid-cols-2">
                                            <div v-for="(value, field) in row" :key="field" class="min-w-0">
                                                <dt class="font-bold text-orange-200">{{ field }}</dt>
