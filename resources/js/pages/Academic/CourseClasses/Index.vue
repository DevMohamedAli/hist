...
<template>
    ...
    <div
        class="academic-course-classes-page h-full min-h-screen w-full flex-1 bg-linear-to-br from-slate-50 to-blue-50/30 p-4 md:p-6"
        dir="rtl"
    >
        <div
        ...
        >
            ...
            >
                <div>
                    <h1
                        class="flex items-center gap-2 text-3xl font-extrabold text-white drop-shadow-md"
                    >
                        <BookOpen
                            class="h-7 w-7 animate-pulse text-orange-300"
                        />
                        إسناد المحاضرين للشعب الدراسية
                    </h1>
                    <p class="mt-1 text-blue-100">
                        كل مقرر داخل مجموعة دراسية يجب أن يكون مسنداً لمحاضر
                        واحد
                    </p>
                </div>
                <div class="flex items-center gap-3">
                    <div
                        class="rounded-2xl bg-white/20 px-5 py-2 text-center backdrop-blur-sm"
                    >
                    ...
                    <Button
                        ...
                        class="gap-2 bg-orange-500 text-white shadow-lg transition-all hover:bg-orange-600"
                    >
                        <Plus class="h-4 w-4" />
                        إسناد جديد
                    </Button>
        ...
        <div
            v-if="successMessage"
            class="mb-5 animate-in duration-300 fade-in slide-in-from-top-2"
        >
            <div
            ...
            >
                <div class="w-full overflow-x-auto">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow
                                v-for="(classItem, idx) in classes"
                                :key="classItem.id"
        ...
        <div class="space-y-3 md:hidden">
            ...
            >
                ...
                <div class="mt-2 flex justify-between">
                    <Badge
                        :class="getStudentBadgeClass(classItem.student_count)"
                        >{{ classItem.student_count }} طالب
                    </Badge>
                </div>
            </div>
            <div v-if="classes.length === 0" class="py-16 text-center">
                ...
                <Button @click="openCreateModal" variant="outline" class="mt-4"
                    >إضافة إسناد جديد</Button
                >
            </div>
        </div>
        ...
        <Transition name="float-button">
            <button
                v-if="isScrolled"
                @click="openCreateModal"
                class="fixed bottom-6 left-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-orange-500 text-white shadow-2xl transition-all hover:scale-110 hover:bg-orange-600"
                title="إسناد جديد"
            >
                <Plus class="h-6 w-6" />
            </button>
        ...
        <Dialog v-model:open="showDeleteModal">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        هل أنت متأكد من حذف إسناد مقرر "{{ deletingName }}"؟ هذا
