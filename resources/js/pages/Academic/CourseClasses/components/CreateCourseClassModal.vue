            ...
            <div
                ...
                <DialogHeader class="relative z-10 text-right space-y-1">
                    <DialogTitle class="flex items-center gap-2 text-xl font-extrabold text-white drop-shadow-sm">
                        <Plus class="h-5 w-5 text-orange-300 animate-pulse" />
                        إسناد محاضر جديد
                    </DialogTitle>
                    <DialogDescription class="text-blue-100 text-xs">قم باختيار المقرر، المجموعة الدراسية، وتعيين
                        المحاضر المناسب</DialogDescription>
                </DialogHeader>
            </div>
            ...
            <form @submit.prevent="submit" class="p-6 space-y-5 bg-white dark:bg-gray-900">
                <div class="space-y-2">
                    ...
                    <Select v-model="form.study_group_id">
                        <SelectTrigger
                            class="w-full transition-all border-gray-200/80 focus:ring-2 focus:ring-orange-500/20">
                            <SelectValue placeholder="اختر المجموعة" />
                        </SelectTrigger>
                        <SelectContent dir="rtl">
                            <SelectItem v-for="g in props.studyGroups" :key="g.id" :value="String(g.id)">
                                {{ g.group_name }} — {{ g.specialization?.name }} <span class="text-xs text-gray-400"
                                    v-if="g.academic_semester?.code">({{ g.academic_semester?.code }})</span>
                            </SelectItem>
                        </SelectContent>
                    </Select>
                ...
                <div class="space-y-2">
                    ...
                    <Select v-model="form.instructor_id">
                        ...
                        <SelectContent dir="rtl">
                            <SelectItem v-for="i in props.instructors" :key="i.id" :value="String(i.id)">
                ...
                <DialogFooter
                    ...
                    <Button type="submit" :disabled="form.processing"
                        class="bg-orange-500 text-white shadow-lg hover:bg-orange-600 transition-all px-6">
                        حفظ الإسناد
                    </Button>
                </DialogFooter>
            </form>
