...
const courseName = computed(
    () => props.courseClass.course?.name ?? 'مقرر غير محدد',
);
const courseCode = computed(() => props.courseClass.course?.code ?? '-');
const courseUnits = computed(() => props.courseClass.course?.units ?? 0);
const groupName = computed(
    () =>
        props.courseClass.study_group?.group_name ??
        props.courseClass.group_name ??
        '-',
);
const specializationName = computed(
    () => props.courseClass.study_group?.specialization?.name ?? '-',
);
const semesterCode = computed(() => props.courseClass.semester?.code ?? '-');
const instructorName = computed(
    () => props.courseClass.instructor?.name ?? 'غير محدد',
);

const gradedCount = computed(
    () =>
        localEnrollments.value.filter(
            (enrollment) =>
...
const passedCount = computed(
    () =>
        localEnrollments.value.filter(
            (enrollment) => computedStatus(enrollment) === 'Passed',
        ).length,
);
...
const failedCount = computed(
    () =>
        localEnrollments.value.filter(
            (enrollment) => computedStatus(enrollment) === 'Failed',
        ).length,
);
...
const totalFor = (enrollment: Enrollment) => {
    return Math.min(
        100,
        numberValue(enrollment.semester_work_mark) +
            numberValue(enrollment.final_exam_mark),
    );
};
...
const computedStatus = (enrollment: Enrollment) => {
    ...
    if (
        total === 0 &&
        finalExam === 0 &&
        numberValue(enrollment.semester_work_mark) === 0 &&
    ...
    router.post(
        ...
        },
        {
            ...
            onSuccess: () => {
                ...
                enrollment.total_mark = Number(
                    flash?.total_mark ?? totalFor(enrollment),
                );
                enrollment.grade_evaluation =
                    flash?.grade_evaluation ??
                    evaluationFor(enrollment.total_mark);
...
<template>
    <Head :title="`رصد الدرجات | ${courseName}`" />
    ...
    <main class="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8" dir="rtl">
        <div class="mx-auto max-w-7xl space-y-6">
                ...
                <div class="border-t-4 border-blue-800 p-5 sm:p-6">
                    ...
                    >
                        <div class="min-w-0">
                            ...
                            <h1
                                class="mt-1 text-2xl font-extrabold text-blue-950 sm:text-3xl"
                            >
                                {{ courseName }}
                            </h1>

                            <p class="mt-2 text-sm text-gray-600">
                                أعمال الفصل من 40، الامتحان النهائي من 60، ولا
                                يعد الطالب ناجحاً إلا إذا حصل على 30 فأكثر في
                                النهائي.
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 text-center">
                            ...
                            >
                                ...
                                <p
                                    class="mt-1 text-xl font-extrabold text-red-950"
                                >
                                    {{ failedCount }}
                                </p>
                            </div>
                    ...
                    <div
                        class="mt-5 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5"
                    >
                    ...
                    <div
                        class="mt-3 flex flex-wrap gap-2 text-xs font-bold text-gray-600"
                    >
                        <span
                            class="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 text-blue-800"
                        >
                        ...
                        <span
                            class="inline-flex items-center gap-1 rounded-full bg-orange-50 px-3 py-1 text-orange-700"
                        >
                        ...
                        <span
                            class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-3 py-1 text-gray-700"
                        >
                        ...
                        >
                            ...
                            مرصود {{ gradedCount }} من
                            {{ localEnrollments.length }}
                        </span>
                    </div>

                    <div
                        class="mt-4 rounded-md border p-3 text-sm font-bold"
                        :class="
                    ...
                    >
                        {{
                            props.gradeEntryAvailability?.is_open
                                ? 'الرصد مفتوح لهذه الشعبة ضمن نافذة الامتحانات النهائية.'
                                : 'هذه الصفحة متاحة فقط بتجاوز إداري لأن نافذة الرصد مغلقة حالياً.'
                        }}
                    </div>
                </div>
            </section>
            ...
            >
                <div class="overflow-x-auto">
                    <table class="w-full min-w-245 text-right text-sm">
                        <thead class="bg-blue-900 text-white">
                            <tr>
                                <th class="px-4 py-4 font-bold">رقم القيد</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            >
                                <td
                                    class="px-4 py-4 font-mono font-bold text-blue-800"
                                >
                                ...
                                <td
                                    class="px-4 py-4 text-center text-lg font-extrabold text-blue-900"
                                >
                                    {{ totalFor(enrollment) }}
                                </td>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <button
                                        ...
                                        :disabled="
                                            !!savingStates[enrollment.id] ||
                                            !!rowError(enrollment)
                                        "
                                        @click="saveGrade(enrollment)"
                                    >
                                        <Loader2
                                            v-if="savingStates[enrollment.id]"
                            ...
                            <tr v-if="localEnrollments.length === 0">
                                ...
                                >
                                    ...
                                    />
                                    لا يوجد طلاب مسجلون في هذه الشعبة حالياً.
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div
            class="pointer-events-none fixed bottom-6 left-6 z-50 w-full max-w-sm"
        >
            <Transition
