...
const permissionLabel = (permission: string) =>
    props.permissionLabels[permission] ?? permission;
const permissionDescription = (permission: string) =>
    props.permissionDescriptions[permission] ?? permission;
...
const requiresLinkedRecord = computed(() =>
    ['student', 'instructor'].includes(newUserForm.linked_type),
);

const linkedRecordPlaceholder = computed(() => {
    if (newUserForm.linked_type === 'student') {
        return linkOptions.value.length > 0
            ? 'اختر الطالب المراد ربطه'
            : 'لا يوجد طلاب غير مرتبطين بحساب';
    }

    if (newUserForm.linked_type === 'instructor') {
        return linkOptions.value.length > 0
            ? 'اختر المحاضر المراد ربطه'
            : 'لا يوجد محاضرون غير مرتبطين بحساب';
    }
                ...
                <div class="border-t-4 border-blue-800 p-5 sm:p-6">
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <div
                                class="rounded-md bg-blue-50 p-3 text-blue-800"
                            >
                            ...
                            <div>
                                ...
                                <p
                                    class="mt-2 max-w-3xl text-sm leading-7 text-gray-600"
                                >
                                    يمكن للمدير العام منح أكثر من دور للمستخدم،
                                    وتعديل صلاحيات كل دور أو إضافة صلاحيات
                                    مباشرة للمستخدم.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 text-center">
                            ...
                            >
                                ...
                                <p
                                    class="mt-1 text-xl font-extrabold text-blue-950"
                                >
                                    {{ users.length }}
                                </p>
                            </div>
                            <div
                                class="rounded-md border border-green-100 bg-green-50 px-4 py-3"
                            >
                                <p class="text-xs font-bold text-green-700">
                                    الأدوار
                                </p>
                                <p
                                    class="mt-1 text-xl font-extrabold text-green-950"
                                >
                                    {{ roles.length }}
                                </p>
                            </div>
                            <div
                                class="rounded-md border border-orange-100 bg-orange-50 px-4 py-3"
                            >
                                <p class="text-xs font-bold text-orange-700">
                                    الصلاحيات
                                </p>
                                <p
                                    class="mt-1 text-xl font-extrabold text-orange-950"
                                >
                                    {{ permissions.length }}
                                </p>
                            </div>
            ...
            >
                ...
                <div class="mt-4 grid gap-4 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        />
                        <p
                            v-if="newUserForm.errors.password"
                            class="mt-1 text-xs font-bold text-red-600"
                        >
                ...
                <div
                    class="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] lg:items-start"
                >
                    <div>
                        <select
                            v-model="newUserForm.linked_id"
                            :disabled="!requiresLinkedRecord"
                            class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:border-blue-700 focus:ring-2 focus:ring-blue-700/15 focus:outline-none disabled:bg-gray-50 disabled:text-gray-400"
                        >
                            <option value="">
                                {{ linkedRecordPlaceholder }}
                            </option>
                            <option
                                v-for="record in linkOptions"
                                :key="record.id"
                        ...
                        <p
                            v-if="
                                requiresLinkedRecord && linkOptions.length === 0
                            "
                            class="mt-1 text-xs font-bold text-amber-700"
                        >
                            لا توجد سجلات متاحة للربط حالياً. السجلات المرتبطة
                            بحساب سابق لا تظهر هنا حتى لا يتم ربط طالب أو محاضر
                            بحسابين.
                        </p>
                        <p
                            v-if="newUserForm.errors.linked_id"
                            class="mt-1 text-xs font-bold text-red-600"
                        >
                            {{ newUserForm.errors.linked_id }}
                        </p>
                        <p class="mt-1 text-xs leading-5 text-gray-500">
                            ربط الموظف لا يحتاج جدولاً منفصلاً حالياً، ويدار عبر
                            دور المستخدم.
                        </p>
                    </div>

                    <div
                        class="grid max-h-32 gap-2 overflow-y-auto rounded-md border border-gray-100 p-2 sm:grid-cols-2"
                    >
                        <label
                            v-for="role in roles"
                            :key="role.id"
                    ...
                    <button
                        ...
                        :disabled="
                            ...
                            (requiresLinkedRecord && !newUserForm.linked_id)
                        "
                        @click="createUser"
                    >
                        <Plus class="h-4 w-4" />
            ...
            <section class="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
                    ...
                    <div class="max-h-[640px] overflow-y-auto p-3">
                        ...
                        >
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <UserCog
                                    class="h-5 w-5 shrink-0 text-blue-700"
                                />
                            </div>
                ...
                <div class="space-y-6">
                    ...
                    >
                        ...
                        <div
                            v-if="selectedUser"
                            class="grid gap-5 p-4 lg:grid-cols-2"
                        >
                            <div>
                                <h3
                                    class="mb-3 text-sm font-extrabold text-gray-900"
                                >
                                    الأدوار
                                </h3>
                            ...
                            <div>
                                ...
                                <div
                                    class="grid max-h-72 gap-2 overflow-y-auto rounded-md border border-gray-100 p-2 sm:grid-cols-2"
                                >
                                    <label
                                    ...
                                    >
                                        <input
                                            ...
                                            :checked="
                                                userForm.direct_permissions.includes(
                                                    permission,
                                                )
                                            "
                                            @change="
                                                toggleIn(
                                                    userForm.direct_permissions,
                                                    permission,
                                                )
                                            "
                                        />
                                        <span class="min-w-0">
                                            ...
                                            >
                                            <span
                                                class="block text-[11px] leading-5 text-gray-500"
                                            >
                                                {{
                    ...
                    >
                        ...
                        <div
                            class="grid gap-4 p-4 lg:grid-cols-[260px_minmax(0,1fr)]"
                        >
                            <div class="space-y-2">
                                <button
                                    v-for="role in roles"
                            ...
                            <div v-if="selectedRole" class="space-y-4">
                                <div
                                    class="rounded-md border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-900"
                                >
                                    <strong>{{
                                    ...
                                    <span
                                        ...
                                        "
                                    >
                                        يحصل دائماً على كل الصلاحيات، حتى لو
                                        تغيرت القائمة.
                                    </span>
                                </div>

                                <div
                                    class="grid max-h-80 gap-2 overflow-y-auto rounded-md border border-gray-100 p-3 sm:grid-cols-2 lg:grid-cols-3"
                                >
                                    <label
                                        v-for="permission in permissions"
                                        :key="permission"
                    ...
                    >
                        ...
                        <div
                            class="mt-4 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)_auto] lg:items-start"
                        >
                            <div>
                                <input
                            ...
                            <div
                                class="grid max-h-32 gap-2 overflow-y-auto rounded-md border border-gray-100 p-2 sm:grid-cols-2 lg:grid-cols-3"
                            >
                                <label
                                    v-for="permission in permissions"
                                    :key="permission"
                                ...
                                >
                                    <input
                                        ...
                                        class="rounded border-gray-300 text-orange-600"
                                        :checked="
                                            newRoleForm.permissions.includes(
                                                permission,
                                            )
                                        "
                                        @change="
                            ...
                            <button
                                ...
                                class="inline-flex items-center justify-center gap-2 rounded-md bg-orange-500 px-5 py-2.5 text-sm font-bold text-white hover:bg-orange-600 disabled:opacity-60"
                                :disabled="
                                    newRoleForm.processing || !newRoleForm.name
                                "
                                @click="createRole"
                            >
                                <Plus class="h-4 w-4" />
