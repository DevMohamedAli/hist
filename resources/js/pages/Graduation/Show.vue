...
const completionPercent = computed(() => {
    ...
    return Math.min(
        100,
        Math.round((props.eligibility.passed_courses.length / required) * 100),
    );
});

const blockedCount = computed(
    () =>
        props.eligibility.missing_courses.length +
        props.eligibility.failed_courses.length,
);
...
<template>
    ...
    <main class="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8" dir="rtl">
        <div class="mx-auto max-w-7xl space-y-6">
            <Link
                href="/graduations"
                class="inline-flex items-center gap-2 text-sm font-bold text-slate-700 hover:text-sky-800"
            >
            ...
            >
                <div
                    class="border-b border-slate-200 bg-slate-900 px-6 py-5 text-white"
                >
                    <div
                    ...
                    >
                        <div class="min-w-0">
                            ...
                            <h1
                                class="mt-2 text-2xl font-black tracking-normal break-words text-white sm:text-3xl"
                            >
                                {{ student.full_name }}
                            </h1>
                ...
                <div
                    v-if="graduationRecord"
                    class="grid gap-3 border-b border-emerald-100 bg-emerald-50 px-6 py-4 text-sm md:grid-cols-3"
                >
                    <div>
                        <span class="text-emerald-700">رقم الشهادة</span>
                        <p
                            class="mt-1 font-mono font-bold text-emerald-950"
                            dir="ltr"
                        >
                            {{ graduationRecord.certificate_number }}
                        </p>
                    </div>
                    <div>
                        <span class="text-emerald-700">تاريخ التخرج</span>
                        <p class="mt-1 font-bold text-emerald-950">
                            {{ graduationRecord.graduation_date }}
                        </p>
                    </div>
                    <div>
                ...
                <div class="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-4">
                    <div class="rounded-md border border-slate-200 p-4">
                        ...
                        <p
                            class="mt-3 flex items-center gap-2 text-lg font-black"
                            :class="
                    ...
                    <div class="rounded-md border border-slate-200 p-4">
                        ...
                        <p class="mt-2 text-xs text-slate-500">
                            {{ eligibility.passed_courses.length }} من
                            {{ eligibility.required_courses.length }} مقرر
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section
                v-if="eligibility.reasons.length"
            ...
            >
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <ul
                            class="mt-2 list-inside list-disc space-y-1 text-sm"
                        >
                            <li
            ...
            <section class="grid gap-6 lg:grid-cols-2">
                ...
                >
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span
                            ...
                            >{{ eligibility.missing_courses.length }}</span
                        >
                    </div>
                    <div class="mt-4 max-h-[360px] overflow-auto">
                        <table class="w-full min-w-[560px] text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr
                                    v-for="course in eligibility.missing_courses"
                                    :key="course.id"
                                >
                                    <td
                                    ...
                                    <td
                                        class="px-3 py-2 font-bold text-slate-900"
                                    >
                                        {{ course.name }}
                                    </td>
                                ...
                                <tr
                                    v-if="
                                        eligibility.missing_courses.length === 0
                                    "
                                >
                                    <td
                                        colspan="4"
                ...
                >
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span
                            class="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700"
                            >{{ eligibility.failed_courses.length }}</span
                        >
                    </div>
                    <div class="mt-4 max-h-[360px] overflow-auto">
                        <table class="w-full min-w-[560px] text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr
                                    v-for="course in eligibility.failed_courses"
                                    :key="course.id"
                                >
                                    <td
                                        class="px-3 py-2 font-mono text-xs"
                                ...
                                <tr
                                    v-if="
                                        eligibility.failed_courses.length === 0
                                    "
                                >
                                    <td
                                        colspan="4"
                ...
                >
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            {{ enrollments.length }} مقرر مسجل، و
                            {{ blockedCount }} مقرر يحتاج متابعة قبل الاعتماد.
                        </p>
                    </div>
                </div>
                    ...
                    <table class="w-full min-w-[980px] text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            >
                                ...
                                <td class="px-3 py-3 text-center">
                                    {{ enrollment.semester_work }}
                                </td>
                                <td class="px-3 py-3 text-center">
                                    {{ enrollment.final_exam }}
                                </td>
                            ...
                            <tr v-if="enrollments.length === 0">
                                ...
                                >
                                    لا توجد مقررات مسجلة لهذا الطالب.
                                </td>
                            </tr>
                        </tbody>
                    </table>
