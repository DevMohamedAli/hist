...
const defaultHeroImage =
    props.settings.hero_image_url || '/assets/img/website-news-hero.svg';
const instituteName = computed(
    () =>
        props.settings.site_name ||
        props.settings.institute_name ||
        'المعهد العالي',
);
const heroNews = computed(() => props.news[0]);
const heroImage = computed(
    () => heroNews.value?.featured_image_url || defaultHeroImage,
);
        ...
        : [
              {
                  ...
                  slug: '',
                  summary:
                      'يمكن لموظف إدارة الموقع إضافة أخبار جديدة من لوحة التحكم وستظهر تلقائياً في هذا الشريط.',
...
const registrationHref = computed(
    () => props.registration.registration_url || '#admission',
);
...
const newsHref = (post: { slug?: string }) =>
    post.slug ? `/news/${post.slug}` : '#news';
const isExternalUrl = (url?: string | null) =>
    Boolean(url && /^https?:\/\//i.test(url));
const linkTarget = (url?: string | null) =>
    isExternalUrl(url) ? '_blank' : undefined;
...
const contactInfo = computed(() => [
    {
        ...
        label: 'الهاتف',
        value:
            props.settings.phone || props.settings.contact_phone || 'غير مضاف',
    },
    {
        icon: Mail,
        label: 'البريد الإلكتروني',
        value:
            props.settings.email || props.settings.contact_email || 'غير مضاف',
    },
]);
...
<template>
    ...
    <PublicLayout>
        <main class="bg-slate-50 text-slate-900">
            <!-- Hero -->
            <section
                class="hero-modern relative isolate overflow-hidden text-white"
            ...
            >
                ...
                <div
                    class="hero-floating hero-floating--book"
                    aria-hidden="true"
                >
                    <GraduationCap class="h-7 w-7" />
                </div>
                <div
                    class="hero-floating hero-floating--spark"
                    aria-hidden="true"
                >
                ...
                <nav
                    class="hero-utility relative mx-auto max-w-7xl px-4 pt-6 sm:px-6"
                ...
                >
                    <div class="hero-copy">
                        ...
                        <h1
                            id="home-hero-title"
                            class="mt-5 max-w-4xl text-4xl leading-tight font-black tracking-tight md:text-6xl"
                        >
                            {{
                                props.settings.hero_title ||
                                'بوابة المعهد للأخبار والخدمات الأكاديمية'
                            }}
                        </h1>

                        <p class="hero-subtitle">
                            تابع آخر الأخبار والفعاليات، استكشف الأقسام العلمية،
                            وانتقل بسهولة إلى خدمات الطالب والموظف وعضو هيئة
                            التدريس.
                        </p>
                        ...
                        >
                            <component
                                :is="
                                    isExternalUrl(registrationHref) ? 'a' : Link
                                "
                                :href="registrationHref"
                                :target="linkTarget(registrationHref)"
                                :rel="
                                    isExternalUrl(registrationHref)
                                        ? 'noopener noreferrer'
                                        : undefined
                                "
                            ...
                            <component
                                :is="
                                    isExternalUrl(props.portalLoginUrl)
                                        ? 'a'
                                        : Link
                                "
                                :href="props.portalLoginUrl"
                                :target="linkTarget(props.portalLoginUrl)"
                                :rel="
                                    isExternalUrl(props.portalLoginUrl)
                                        ? 'noopener noreferrer'
                                        : undefined
                        ...
                        <div class="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
                            ...
                            >
                                ...
                                <p class="text-xs font-bold text-blue-50/80">
                                    {{ stat.label }}
                                </p>
                            </div>
                        </div>
                    ...
                    <aside
                        class="hero-visual"
                        aria-label="مختصر أخبار وحالة التسجيل"
                    >
                        <div class="hero-image-card">
                            <img
                                :src="heroImage"
                                :alt="heroNews?.title || 'أخبار المعهد'"
                            ...
                            <div
                                class="absolute right-4 bottom-4 left-4 rounded-3xl border border-white/15 bg-white/15 p-4 backdrop-blur-xl"
                            >
                                <p
                                    class="flex items-center gap-2 text-xs font-extrabold text-orange-200"
                                >
                                    <Clock3 class="h-4 w-4" />
                                    مختصر اليوم
                                </p>
                                <h2
                                    class="mt-1 line-clamp-2 text-lg font-black"
                                >
                                    {{
                                        heroNews?.title ||
                                        'أخبار المعهد تظهر هنا'
                                    }}
                                </h2>
                                <Link
                                    v-if="heroNews?.slug"
                                    :href="newsHref(heroNews)"
                        ...
                        <div class="hero-status-card">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    <p
                                        class="text-sm font-extrabold text-orange-200"
                                    >
                                        حالة التسجيل
                                    </p>
                            ...
                            <p class="mt-4 text-sm leading-7 text-blue-50/85">
                                <span v-if="props.registration.semester">
                                    الفصل:
                                    {{ props.registration.semester.code }} ·
                                    نهاية التسجيل:
                                    {{
                                        formatDate(
                                            props.registration.semester
                                                .registration_end,
                                        )
                                    }}
                                </span>
                        ...
                        >
                            <Link :href="newsHref(post)" class="block">
                                <div class="relative h-48 overflow-hidden">
                                    ...
                                    />
                                    <span
                                        class="absolute top-3 right-3 rounded-full bg-white/90 px-3 py-1 text-xs font-black text-blue-950 shadow"
                                    >
                                        خبر
                                    </span>
                                </div>
                                ...
                                <div class="p-5">
                                    <h3
                                        class="line-clamp-2 text-lg leading-7 font-black text-blue-950 group-hover:text-orange-600"
                                    >
                                        {{ post.title }}
                                    </h3>
                                    <p
                                        class="mt-2 line-clamp-3 text-sm leading-7 text-slate-600"
                                    >
                                        {{
                                            post.summary ||
                                            'تفاصيل الخبر ستظهر هنا بعد إضافتها من لوحة الإدارة.'
                                        }}
                                    </p>
                                    <span
            ...
            <!-- Statistics -->
            <section
                class="relative overflow-hidden bg-blue-950 py-16 text-white"
                aria-label="إحصائيات المعهد"
            >
                <img
                    src="/assets/img/website-news-hero.svg"
                    alt=""
                ...
                <div class="relative mx-auto max-w-7xl px-4">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        >
                            <component
                                :is="statistic.icon"
                                class="mx-auto h-9 w-9 text-orange-300"
                            />
                            <p class="mt-5 text-5xl font-black text-orange-300">
                                {{ formatNumber(statistic.value) }}
                            </p>
                            <p class="mt-3 text-lg font-black">
                                {{ statistic.label }}
                            </p>
                            <p class="mt-2 text-sm leading-6 text-blue-50/70">
                                {{ statistic.helper }}
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div class="relative flex h-full flex-col justify-end">
                            ...
                            <p
                                ...
                                class="mt-3 max-w-xl leading-8 text-blue-50/85"
                            >
                                {{ banner.subtitle }}
                            </p>
                            ...
                            <component
                                :is="
                                    isExternalUrl(banner.link_url) ? 'a' : Link
                                "
                                v-if="banner.link_url"
                                :href="banner.link_url"
                                :target="linkTarget(banner.link_url)"
                                :rel="
                            ...
                            >
                                ...
                                <ExternalLink
                                    v-if="isExternalUrl(banner.link_url)"
                                    class="h-4 w-4"
                                />
            ...
            <section class="mx-auto max-w-7xl px-4 py-12">
                ...
                >
                    ...
                    >
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-950"
                        >
                        ...
                        <p
                            class="mt-3 min-h-20 text-sm leading-7 text-slate-600"
                        >
                            {{
                                department.description ||
                                'قسم علمي معتمد ضمن برامج المعهد.'
                            }}
                        </p>
                ...
                <div v-else class="empty-state">
                    ...
                    <h3 class="mt-3 text-xl font-black text-blue-950">
                        لم تتم إضافة أقسام بعد
                    </h3>
                    <p class="mt-2 text-sm text-slate-500">
                        ستظهر الأقسام والتخصصات هنا عند إضافتها من لوحة الإدارة.
                    </p>
                </div>
            </section>
            ...
            <section
                class="mx-auto grid max-w-7xl gap-6 px-4 py-12 lg:grid-cols-[0.9fr_1.1fr]"
            >
                <div class="section-card p-6">
                    ...
                    <div v-if="topAnnouncements.length" class="mt-6 space-y-3">
                        <article
                            v-for="post in topAnnouncements"
                            :key="post.id"
                    ...
                    <p
                        v-else
                        class="mt-6 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500"
                    >
                ...
                <div id="faq" class="section-card p-6">
                    ...
                    <div v-if="topFaqs.length" class="mt-6 space-y-3">
                        ...
                        >
                            ...
                            <p
                                class="mt-3 border-t border-slate-100 pt-3 text-sm leading-8 text-slate-600"
                            >
                                {{ faq.answer }}
                            </p>
                        </details>
            ...
            <section id="admission" class="mx-auto max-w-7xl px-4 py-12">
                <div
                    class="relative overflow-hidden rounded-[2rem] bg-blue-950 p-6 text-white shadow-xl md:p-8"
                >
                    <div
                        class="absolute -top-20 -left-20 h-64 w-64 rounded-full bg-orange-500/20 blur-3xl"
                    />
                    <div
                        class="absolute right-10 -bottom-24 h-72 w-72 rounded-full bg-blue-400/20 blur-3xl"
                    />

                    <div
                        class="relative grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start"
                    >
                        <div>
                            ...
                            <p class="mt-4 leading-8 text-blue-50/80">
                                جهّز المستندات المطلوبة قبل بدء التسجيل لتسهيل
                                عملية القبول وتقليل وقت المراجعة.
                            </p>
                        ...
                        <ul
                            v-if="props.registration.requirements.length"
                            class="relative grid gap-3 md:grid-cols-2"
                        >
                            <li
                                v-for="requirement in props.registration
                                    .requirements"
                                :key="requirement"
                                class="flex gap-3 rounded-2xl border border-white/10 bg-white/10 p-4 leading-7 backdrop-blur"
                            >
                                <CheckCircle2
                                    class="mt-1 h-5 w-5 shrink-0 text-orange-300"
                                />
                        ...
                        <div
                            v-else
                            class="relative rounded-2xl border border-white/10 bg-white/10 p-5 text-blue-50/85"
                        >
                ...
                <div class="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
                    <aside class="section-card p-6">
                        ...
                        <p class="mt-4 leading-8 text-slate-600">
                            ارسل استفسارك وسيتم التواصل معك من الإدارة المختصة
                            في أقرب وقت.
                        </p>
